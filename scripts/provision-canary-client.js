...
const db=getDb(),file=path.join(process.env.DATA_DIR||'/data','.env.gina-canary-harness');
...
const row=rows[0];
if(row&&(row.profile!==CANARY_PROFILE||row.source!==CANARY_SOURCE||row.redirect_uri||!row.client_secret_hash))throw Error('Existing identity does not match dedicated read-only contract; stop for review.');
const summary=(clientId=row?.client_id)=>({clientIdSha256:clientId?createHash('sha256').update(clientId).digest('hex'):null,clientName:CANARY_CLIENT_NAME,profile:CANARY_PROFILE,source:CANARY_SOURCE,allowedListId:CANARY_LIST_ID,readOnly:true,mutationsEnabled:false,credentialFile:file,secretPrinted:false});
if(command==='--status'){console.log(JSON.stringify({...summary(),registered:!!row,privateCredentialPresent:existsSync(file)}));}
 ...
 if(row){
  if(!existsSync(file)||(statSync(file).mode&0o077))throw Error('Existing client private delivery file missing or insecure; revoke/reprovision explicitly.');
  const env=Object.fromEntries(readFileSync(file,'utf8').trim().split('\n').map(line=>{const i=line.indexOf('=');return [line.slice(0,i),line.slice(i+1)];}));
  if(env.GINA_CANARY_CLIENT_ID!==row.client_id||!await bcrypt.compare(env.GINA_CANARY_CLIENT_SECRET||'',row.client_secret_hash))throw Error('Existing credential file mismatch; stop for review.');
